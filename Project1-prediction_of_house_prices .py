...
#Question1:Drop the columns "id" and "Unnamed: 0" from axis 1 using the method drop(), then use the method describe() to obtain a statistical summary of the data.
import pandas as pd
dff=dff.drop(["id"],axis=1)
dff=dff.drop(["Unnamed: 0"],axis=1)
dff.describe()
#Question2:Fit a linear regression model to predict the 'price' using the feature 'sqft_living'
...
#Question3:Evaluate the model for accuracy using the methods taught in class
from sklearn.model_selection import train_test_split
X_train,X_test,y_train,y_test=train_test_split(dff,dff,test_size=.2,random_state=4);
from sklearn import linear_model
regr = linear_model.LinearRegression()
